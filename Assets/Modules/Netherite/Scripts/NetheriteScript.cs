...
public class NetheriteScript : ModuleScript
{
    ...
    private int NetheriteCount { get { return IsEditor ? 3 : Get<KMBombInfo>().GetSolvableModuleNames().Where(m => m == ModuleName).Count(); } }
    ...
    private void OnInteract(int i)
    {
        ...
        ButtonEffect(Buttons[i], 1, Sounds.N.Dig, KMSoundOverride.SoundEffect.ButtonPress);

        // Ping if this is the first time the module is being cracked.
        if (Stage == 0)
            PlaySound(Sounds.N.Ping);
