...
#pragma warning restore 414
    ...
    private IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        {
            yield return null;
            yield return @"sendtochat NOTE, SETTINGS CANNOT BE CHANGED AFTER MODULE INITIATION! | !{1} set 3-18 (Sets dimension count, in Normal/Time mode you may not start with less than 9) | !{1} spin <#> (Sets rotation count, in Normal/Time mode you may not start with less than 3, sets the amount of rotations) | !{0} slowness <#> (Determines how slow the spheres move where lower = faster movement with each +/-1 being twice as slow/fast, in Normal/Time mode you may not set this to more than 9) | !{0} stay (Toggle: Keeps each sphere's colors as they rotate rather than updating based on position) | !{1} stretch (Toggle: Only affects dimensions not divisible by 3, stretches the X, Y, and Z planes to fit the module, however it causes some axes to become exaggerated) | !{1} springiness (Toggle: Uses InOutBounce ease, do not use on serious TP bombs) | !{1} supple (Toggle: Uses InOutElastic ease, do not use on serious TP bombs)";
        }
        ...
        else if (Regex.IsMatch(split[0], @"^\s*set\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
        {
            ...
            if (Interact.Dimension != 0)
                ...
                yield return "sendtochaterror Huge dimension counts are disabled to reduce lag.";
    ...
    {
        var dimPoints = new Dictionary<int, int>()
        {
            { 3, 13 }, { 4, 22 }, { 5, 28 },
            { 6, 35 }, { 7, 50 }, { 8, 65 },
            { 9, 80 }, { 10, 110 }, { 11, 130 },
