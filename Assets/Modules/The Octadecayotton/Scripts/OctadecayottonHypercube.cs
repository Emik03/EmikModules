...
public class OctadecayottonHypercube : MonoBehaviour
{
    ...
    private static List<Vector4> _basisVectors = Enumerable.Range(0, MaxDimensions)
       .Select(i => new Vector4(Position.weights[i, 0], Position.weights[i, 1], Position.weights[i, 2], 0))
       .ToList();
    ...
    internal IEnumerator AddMeshes(int dim, bool stretch, TheOctadecayottonScript octa)
    {
        ...
        _renderer.material.SetVectorArray(
            "_sphereBasis",
            BaseMesh(dim == 12 ? 1 : dim).Select(v => (Vector4)v).ToList()
        );
        ...
        if (stretch)
        {
            ...
            _sphereA.transform.localScale = new Vector3(1f / maxOffset.x, 1f / maxOffset.y, 1f / maxOffset.z) /
                (dim > LODBarrier ? ScaleFactor * BigScaleFactor : ScaleFactor) *
                2.5f;

            _sphereB.transform.localScale = new Vector3(1f / maxOffset.x, 1f / maxOffset.y, 1f / maxOffset.z) /
                (dim > LODBarrier ? ScaleFactor * BigScaleFactor : ScaleFactor) *
                2.5f;
        }
        else
        {
            ...
            _sphereA.transform.localScale = new Vector3(scale, scale, scale) /
                (dim > LODBarrier ? ScaleFactor * BigScaleFactor : ScaleFactor) *
                2.5f;

            _sphereB.transform.localScale = new Vector3(scale, scale, scale) /
                (dim > LODBarrier ? ScaleFactor * BigScaleFactor : ScaleFactor) *
                2.5f;
        }

        _sphereA.GetComponentInChildren<Light>().range =
            2 / Mathf.Pow(_dimension, 2) * octa.Interact.transform.lossyScale.x;

        _sphereB.GetComponentInChildren<Light>().range =
            2 / Mathf.Pow(_dimension, 2) * octa.Interact.transform.lossyScale.x;

        if (dim == 12 && !_experimentalRendering)
        {
            _filter.sharedMesh =
                _specialMesh = _specialMesh ?? (_asyncSpecialMesh ?? GenerateMeshOfDimensions(12, 1)).Mesh;

            var r2 = Instantiate(_renderer.gameObject, _renderer.transform.parent, true).GetComponent<MeshRenderer>();

            r2.GetComponent<MeshFilter>().sharedMesh = _specialMeshB =
                _specialMeshB ?? (_asyncSpecialMeshB ?? GenerateMeshOfDimensions(12, 2)).Mesh;
        ...
        else
        {
            ...
            for (int i = 1; i <= amountNeeded; i++)
            {
                ...
                _allRenderers[i] = Instantiate(_renderer.gameObject, _renderer.transform.parent, true)
    ...
    private Color ColorFromIndex(int ix)
    {
        ...
        return new Color(
            total.x / MaxAxes(_dimension).x,
            total.y / MaxAxes(_dimension).y,
            total.z / MaxAxes(_dimension).z
        );
    }
    ...
    private IEnumerator FadeInOut(Renderer fader, float endAmount)
    {
        ...
        {
            fader.material.color = new Color(
                1f,
                1f,
                1f,
                Mathf.Lerp(1f - endAmount, endAmount, (Time.time - t) / duration)
            );
    ...
    internal void SetRotation(Rotation[][] rotations)
    {
        ...
        var inv = PadLimit(
                rotations.SelectMany(r => PadLimit(r.Select(x => x.IsNegative ? 1f : 0f), 5)),
                MaxRotations * MaxAxesPerRotation
            )
           .ToArray();

        var rot = PadLimit(
                rotations.SelectMany(r => PadLimit(r.Select(x => (float)(int)x.Axis), 5)),
                MaxRotations * MaxAxesPerRotation
            )
           .ToArray();
    ...
    internal static Vector3[] BaseMesh(int dim)
    {
        if (dim > LODBarrier)
            return new Vector3[]
                {
                    new Vector3(1f, 1f, 0f), new Vector3(1f, 1f, 2f), new Vector3(1f, 0f, 1f), new Vector3(1f, 2f, 1f),
                    new Vector3(0f, 1f, 1f), new Vector3(2f, 1f, 1f)
                }.Select(v => v / ScaleFactor / BigScaleFactor)
               .ToArray();

        return new Vector3[]
            {
                new Vector3(1.000000f, 1.707107f, 0.292893f), new Vector3(1.000000f, 2.000000f, 1.000000f),
                new Vector3(1.000000f, 1.707107f, 1.707107f), new Vector3(1.000000f, 1.000000f, 2.000000f),
                new Vector3(1.672498f, 1.218508f, 0.292893f), new Vector3(1.951056f, 1.309017f, 1.000000f),
                new Vector3(1.672498f, 1.218508f, 1.707107f), new Vector3(1.415627f, 0.427939f, 0.292893f),
                new Vector3(1.587785f, 0.190983f, 1.000000f), new Vector3(1.415627f, 0.427939f, 1.707107f),
                new Vector3(1.000000f, 1.000000f, 0.000000f), new Vector3(0.584373f, 0.427939f, 0.292893f),
                new Vector3(0.412215f, 0.190983f, 1.000000f), new Vector3(0.584373f, 0.427939f, 1.707107f),
                new Vector3(0.327502f, 1.218508f, 0.292893f), new Vector3(0.048943f, 1.309017f, 1.000000f),
    ...
    {
        if (dim > LODBarrier)
            return new int[] { 0, 2, 4, 1, 4, 2, 0, 4, 3, 1, 3, 4, 0, 5, 2, 1, 2, 5, 0, 3, 5, 1, 5, 3 };

        return new int[]
            {
                1, 6, 2, 3, 6, 7, 1, 11, 5, 4, 3, 7, 7, 9, 10, 5, 11, 8, 4, 7, 10, 5, 9, 6, 10, 13, 14, 8, 11, 12, 4,
                10, 14, 8, 13, 9, 13, 15, 16, 13, 17, 14, 12, 11, 15, 4, 14, 17, 4, 17, 3, 15, 2, 16, 16, 3, 17, 15, 11,
                1, 1, 5, 6, 3, 2, 6, 7, 6, 9, 5, 8, 9, 10, 9, 13, 8, 12, 13, 13, 12, 15, 13, 16, 17, 15, 1, 2, 16, 2, 3
            }.Select(i => i - 1)
           .ToArray();
    }
    ...
    private void Awake()
    {
        ...
        if (_experimentalRendering)
            Debug.Log(
                "<The Octadecayotton> Generating meshes on another thread: dimentions 3-18 in high detail, dimensions 19-21 low detail"
            );
        else
            Debug.Log(
                "<The Octadecayotton> Generating meshes on another thread: dimentions 3-11 in high detail, dimension 13 low detail, 2x dimension 12 in high detail"
            );
    ...
    private void GenerateMeshes()
    {
        ...
        for (int dim = 3; dim <= MeshLimit; dim++)
            if (_meshTemplates[dim - 3] == null &&
                _asyncMeshTemplates[dim - 3] == null &&
                (dim != 12 || _experimentalRendering))
    ...
    private static MeshData GenerateMeshOfDimensions(int dim, int special = 0)
    {
        ...
        total = (_experimentalRendering && dim == 21) || (!_experimentalRendering && dim == 13)
            ? MaxAxes(MaxDimensions) +
            new Vector3(2f, 2f, 2f) / (MaxDimensions > LODBarrier ? ScaleFactor * BigScaleFactor : ScaleFactor)
            : total;

        return new MeshData
        {
            Vertices = vertices, Triangles = triangles, UV = uvs, Name = "Octa" +
                dim +
                "d" +
                (special == 0 ? "" :
                    special == 1 ? "A" : "B"),
